...
wss.on('connection', function connection(ws, req) {
    ...
    ws.send(JSON.stringify({ action: 'initMarkers', markers: markers }));
    ...
    ws.on('message', function incoming(data) {
        ...
        if (message.action === 'updateLocation') {
            ...
            const { latitude, longitude, username, userText, messageId } = message;
            const marker = { latitude, longitude, username, userText, messageId };
            markers.push(marker);

            // Broadcast the new marker to all connected clients
            wss.clients.forEach(client => {
                if (client.readyState === WebSocket.OPEN) {
                    client.send(JSON.stringify({ action: 'addMarker', marker: marker }));
                }
            });

        } else if (message.action === 'deleteMarker') {
            const { messageId } = message;
            markers = markers.filter(marker => marker.messageId !== messageId);

            // Broadcast the marker deletion to all connected clients
            wss.clients.forEach(client => {
                if (client.readyState === WebSocket.OPEN) {
                    client.send(JSON.stringify({ action: 'deleteMarker', messageId: messageId }));
                }
            });

            // Also delete the associated chat message
