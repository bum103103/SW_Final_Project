...
function initializeChat(roomId) {
    ...
    socket.on('message2', (data) => {
        addMessageOnlyMap(data.text, data.messageId, data.username);
        scrollToBottom();
    });
...
function addMessageOnlyMap(messageText, messageId, messageUsername) {
...
function sendMessage2() {
    ...
    if (messageText) {
        ...
        socket.emit('message2', messageData);
        messageInput.value = '';
    }
    scrollToBottom();
}
